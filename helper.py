...
---
    ...
    command_string: Modifies a string to start with an exclamation mark (!) if it does not already start with one.
    fix_command: Modifies a list of strings to start with an exclamation mark (!) if they do not already start with one.
...
def random_greeting(name:str="<NAME>") -> str:
    ...
    greetings = list(set(commands.greets() + ["How's it going?", "How are ya?", "How's it hangin'?", "what's up?", "good'ay"]))
    return random.choice(list(map(lambda word: f"{word[1:].capitalize()}, {name.capitalize()}!" if word.startswith("!") else f"{word.capitalize()}, {name.capitalize()}!", greetings))).capitalize()
